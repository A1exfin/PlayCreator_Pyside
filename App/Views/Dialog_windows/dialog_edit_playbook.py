...
class DialogEditPlaybook(QDialog):
    def __init__(self, playbook_name: str, playbook_info: str,
        ...
        self._line_edit_playbook_name.setText(playbook_name)
        self._line_edit_playbook_name.setMaxLength(PLAYBOOK_NAME_MAX_LENGTH)

        self._combo_who_can_edit = QComboBox(parent=self)
        self._combo_who_can_edit.addItems(
        ...
        self._combo_who_can_edit.setCurrentIndex(who_can_edit.value)

        self._combo_who_can_see = QComboBox(parent=self)
        self._combo_who_can_see.addItems(
            ['Глава команды', 'Правление команды (глава и тренеры)',
             'Постоянные игроки команды (все, кроме приглашённых)',
             'Все игроки команды (в том числе приглашённые игроки)', 'Все']
        )
        self._combo_who_can_see.setCurrentIndex(who_can_see.value)
        ...
        vertical_layout.addWidget(self._combo_who_can_edit)
        vertical_layout.addWidget(label_who_can_see)
        vertical_layout.addWidget(self._combo_who_can_see)
    ...
    def get_data(self) -> 'playbook_data':
        return playbook_data(
            self._line_edit_playbook_name.text().strip(), self.text_edit_playbook_info.toPlainText(),
            PlaybookAccessOptions(self._combo_who_can_edit.currentIndex()),
            PlaybookAccessOptions(self._combo_who_can_see.currentIndex())
        )
